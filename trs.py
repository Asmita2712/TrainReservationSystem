...
class GUI:
    ...
    def connect(self):
        try:
            self.db = pymysql.connect(host="localhost",user="root",passwd = "asmita",db="asmita",autocommit=True)
    ...
    def loginPage(self):
        ...
        self.photo = ImageTk.PhotoImage(Image.open("C:/Users/aakam/OneDrive/Desktop/train.jpg"))
        self.pic = Label(self.rootWin, image = self.photo)
        self.pic.grid(row = 0, column = 0, columnspan=4)
        Label(self.rootWin, text="Railway Reservation System", font=("Times New Roman", 18)).grid(row=1,column=0,columnspan=3,sticky=E+W)
    ...
    def selectDeparturePage(self):
        ...
        self.c.execute("CREATE OR REPLACE VIEW GoodTrains AS SELECT TrainNumber, DepartureTime, ArrivalTime FROM GoodArrivals A NATURAL JOIN GoodDeparts D GROUP BY TrainNumber")
    ...
    def makeTravelExtrasPage(self):
        ...
        Label(self.travelExtrasPage, text="4 Bag Max. 2 Free of Charge. Next 2 are Rs.30 per bag.").grid(row=3,column=0,columnspan=2,sticky=E+W)
    ...
    def makeTicketsPage(self):
        ...
        Label(self.ticketsPage, text="No. of Bags").grid(row=3,column=6)
    ...
    def payForReservation(self):
        if self.cardVar.get() != "" and self.cardVar.get() != "No Card Found":
            ...
            if expD < datetime.datetime.now().date():
                ...
                self.c.execute("INSERT INTO Reservation VALUES(%s,%s,%s,%s,%s)",(cardNo,False,self.reservationNumber,self.username,self.totalcost))
    ...
    def updateReservationSearch(self):
        ...
        if num !=0:
            ...
            Label(self.updateSearchPage,text="Departs From").grid(row=1,column=4)
            Label(self.updateSearchPage,text="Departure date").grid(row=1,column=5)
            ...
            Label(self.updateSearchPage,text="No. of Baggages").grid(row=1,column=9)
    ...
    def nextUpdate(self):
        if self.radioVar1.get()==1000:
            ...
            Label(self.nextUpdatePage,text="No. of Baggages").grid(row=1,column=9)
    ...
    def nextUpdate2(self):
            ...
            Label(self.nextUpdatePage2,text="No. of Baggages").grid(row=1,column=9)
            ...
            Label(self.nextUpdatePage2,text="Rs.50").grid(row=5,column=2)
    ...
    def submitReview(self):
        ...
        if cursor5==0:
            ...
            cursor3.execute("Insert into Review(TrainNumber,Comment,ReviewNum,Username,Rating) Values (%s,%s,%s,%s,%s)",(self.trainNumberReview.get(),self.commentReview.get(),self.reviewNum,self.username,self.reviewVar.get()))
